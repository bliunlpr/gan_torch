...
from data.audioparse import load_audio_feat_len
...
class SequentialDatasetWithLength(SequentialDataset):
    def __init__(self, *args, **kwargs):
        ...
        audio_paths = [path for (_, path) in self.utt_ids]
        audio_lengths = [load_audio_feat_len(path) for path in audio_paths]
